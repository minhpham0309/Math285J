...
from .FISTA_fast import _update_FISTA_fast
...
def _update_FISTA(X, W, dW, Ht, diagonal, theta, l1_reg, l2_reg, shuffle,
                               random_state):
    ...
    # The following seems to be required on 64-bit Windows w/ Python 3.5.
    permutation = np.asarray(permutation, dtype=np.intp)
    return _update_FISTA_fast(W, dW, HHt, XHt, diagonal, theta, permutation)


def _fit_FISTA(X, W, H, tol=1e-4, max_iter=200, alpha=0.001,
                            l1_ratio=0., regularization=None, update_H=True,
                            verbose=0, shuffle=False, random_state=None):
    """Compute Non-negative Matrix Factorization (NMF) with FISTA & Coordinate Descent"""

    # so W and Ht are both in C order in memory
    Ht = check_array(H.T, order='C')
    X = check_array(X, accept_sparse='csr')

    # create dW = W_n+1 - W_n, d_H
    dW  = np.zeros(W.shape)
    n_components, n_features = H.shape
    dHt = np.zeros((n_features, n_components))

    # initialize diagonal
    diagonal_W = np.ascontiguousarray(fast_dot(W.T, W).diagonal(),dtype=np.float64)
    diagonal_H = np.ascontiguousarray(fast_dot(H, Ht).diagonal(),dtype=np.float64)
    #np.ascontiguousarray(x, dtype=np.float32)

    # initialize theta
    theta_W = 0.3*np.ones(W.shape[1])
    theta_H = theta_W
    ...
    for n_iter in range(max_iter):
        ...
        # Update W
        violation += _update_FISTA(X, W, dW, Ht, diagonal_H, theta_W,
                                   l1_W, l2_W, shuffle, rng)
        # Update H
        if update_H:
            violation += _update_FISTA(X.T, Ht, dHt, W, diagonal_W, theta_H,
                                   l1_H, l2_H, shuffle, rng)
        ...
        if verbose:
            print(n_iter,"violation:", violation / violation_init, ' abs violation', violation)
...
def non_negative_factorization(X, W=None, H=None, n_components=None,
    ...
    if solver == 'pg':
        ...
        W, H, n_iter = _fit_FISTA(X, W, H, tol,
                                               max_iter,
                                               alpha, l1_ratio,
                                               regularization,
                                               update_H=update_H,
